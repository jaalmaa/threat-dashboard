...
sample_request = {}
sample_request['honeypot_data'] = COWRIE_INTERACTION_TEST_DATA

creds = COWRIE_INTERACTION_TEST_DATA['credentials']['username'] + ':' + COWRIE_INTERACTION_TEST_DATA['credentials']['password']
sample_request['credentials'] = creds
...
def test_response_code_for_valid_data():
    ...
    print(json.dumps(COWRIE_INTERACTION_TEST_DATA))
    response = requests.post(url=url, data=json.dumps(sample_request), headers=headers)
...
def test_response_code_for_invalid_data():
    ...
    url = ANALYTICS_ENGINE_URI + '/analyze'
    headers = { 'Content-Type': 'application/json' }
    response = requests.post(url=url, data=sample_request, headers=headers)
